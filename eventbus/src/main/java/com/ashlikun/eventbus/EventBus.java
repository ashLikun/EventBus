...
public class EventBus {
    ...
    private void subscribe(Object subscriber, SubscriberMethod subscriberMethod) {
        ...
        if (subscriberMethod.sticky) {
            if (eventInheritance) {
                ...
                Set<Map.Entry<Class<?>, EventData>> entries = stickyEvents.entrySet();
                for (Map.Entry<Class<?>, EventData> entry : entries) {
                    Class<?> candidateEventType = entry.getKey();
                    if (eventType.isAssignableFrom(candidateEventType)) {
                        EventData stickyEvent = entry.getValue();
                        checkPostStickyEventToSubscription(newSubscription, stickyEvent);
                    }
                }
            } else {
                EventData stickyEvent = stickyEvents.get(eventType);
                checkPostStickyEventToSubscription(newSubscription, stickyEvent);
            }
        }
    }
    ...
    public void post(Object event, String tag) {
        ...
        List<EventData> eventQueue = postingState.eventQueue;
        EventData eventType = new EventData(event, tag);
    ...
    public void postSticky(Object event) {
        postSticky(event, SubscriberMethod.DEFAULT_TAG);
    }

    public void postSticky(Object event, String tag) {
        synchronized (stickyEvents) {
            stickyEvents.put(event.getClass(), new EventData(event, tag));
        }
    ...
    private void postSingleEvent(EventData eventType, PostingThreadState postingState) throws Error {
        Class<?> paramClass = eventType.event.getClass();
        boolean subscriptionFound = false;
        if (eventInheritance) {
            List<Class<?>> eventTypes = lookupAllEventTypes(paramClass);
            int countTypes = eventTypes.size();
            for (int h = 0; h < countTypes; h++) {
                Class<?> clazz = eventTypes.get(h);
                subscriptionFound |= postSingleEventForEventType(eventType, postingState, clazz);
            }
        } else {
            subscriptionFound = postSingleEventForEventType(eventType, postingState, paramClass);
        }
        if (!subscriptionFound) {
            if (logNoSubscriberMessages) {
                Log.d(TAG, "No subscribers registered for event " + paramClass);
            }
            if (sendNoSubscriberEvent && paramClass != NoSubscriberEvent.class &&
                    paramClass != SubscriberExceptionEvent.class) {
                post(new NoSubscriberEvent(this, eventType.event));
            }
        }
    }
    ...
    private boolean postSingleEventForEventType(EventData eventType, PostingThreadState postingState, Class<?> eventClass) {
        ...
        if (subscriptions != null && !subscriptions.isEmpty()) {
            ...
            for (Subscription subscription : subscriptions) {
                if (subscription.subscriberMethod != null &&
                        TextUtils.equals(eventType.tag, subscription.subscriberMethod.tag)) {
                    resault = true;
    ...
    final static class PostingThreadState {
        final List<EventData> eventQueue = new ArrayList<EventData>();
        boolean isPosting;
